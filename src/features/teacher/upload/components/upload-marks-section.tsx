...
import {
  marksCsvRowSchema,
} from "@/features/teacher/upload/schemas/upload.schema";
import { parseCsvFile } from "@/features/teacher/upload/utils/csv";
import { logUploadHistory } from "@/services/upload-history.service";
import { uploadMarksRows } from "@/services/upload-marks.service";
import type { MarksCsvRow } from "@/types/upload";

type UploadMarksSectionProps = {
  teacherId: string;
  onUploaded: () => void;
};

export function UploadMarksSection({
  teacherId,
  onUploaded,
}: UploadMarksSectionProps) {
  const { toast } = useToast();
  const [preview, setPreview] = useState<MarksCsvRow[]>([]);
  ...
  const handleFile = async (file: File | null) => {
    ...
    try {
      const result = await parseCsvFile<MarksCsvRow>(file, (row) => {
        const parsed = marksCsvRowSchema.safeParse(row);
        if (!parsed.success) {
          return {
  ...
  const handleUpload = async () => {
    ...
    try {
      const result = await uploadMarksRows(preview, teacherId);
      setProgress(100);
  ...
  return (
    <Card>
      <CardContent className="space-y-4 py-5">
        <div>
          <h3 className="font-heading text-lg font-semibold">Upload marks</h3>
          <Text variant="muted" className="mt-1">
            CSV columns: studentId, studentName, subject, marksObtained,
            totalMarks, examType, date
          </Text>
        </div>

        <div className="space-y-2">
          <Label htmlFor="marks-csv">CSV file</Label>
          <input
            id="marks-csv"
            type="file"
            accept=".csv,text/csv"
        ...
        {errors.length > 0 ? (
          ...
          >
            ...
            {errors.length > 8 ? (
              <p className="mt-1">+{errors.length - 8} more errors</p>
            ) : null}
          </div>
        ) : null}
              ...
              <tbody>
                {preview.slice(0, 10).map((row, i) => (
                  <tr key={`${row.studentId}-${i}`} className="border-t border-border/60">
                    ...
                    <td className="px-2 py-1.5">{row.marksObtained}</td>
                    <td className="px-2 py-1.5">{row.totalMarks}</td>
                    <td className="px-2 py-1.5">{row.examType}</td>
